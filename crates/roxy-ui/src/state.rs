...
pub struct AppState {
    ...
    /// Timestamp of the last time the UI was cleared
    /// Used to filter out old records from ClickHouse
    pub clear_ts: i64,
}
...
impl AppState {
        ...
        match msg {
            UiMessage::RequestsUpdated(requests) => {
                self.requests = requests
                    .into_iter()
                    .filter(|r| r.timestamp > self.clear_ts)
                    .collect();
            }
            UiMessage::HostsUpdated(hosts) => {
                self.hosts = hosts
                    .into_iter()
                    .filter(|h| h.last_seen > self.clear_ts)
                    .collect();
            }
            UiMessage::ServicesUpdated(client_services) => {
                ...
                self.services = client_services
                    .into_iter()
                    .filter(|cs| cs.last_seen > self.clear_ts)
            ...
            UiMessage::RumViewsUpdated(views) => {
                self.rum_views = views
                    .into_iter()
                    .filter(|v| v.timestamp > self.clear_ts)
                    .collect();
            }
    ...
    pub fn clear(&mut self) {
        ...
        info!("UI cleared at timestamp: {}", self.clear_ts);
    }
